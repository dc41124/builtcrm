  ...
  try {
    ...
    if (
      ...
      throw new AuthorizationError(
        "Only the client can reject approvals",
        "forbidden",
      );
    }
    ...
    await db.transaction(async (tx) => {
      ...
      await writeActivityFeedItem(
        ctx,
        {
          ...
          summary: `APV-${String(apv.approvalNumber).padStart(3, "0")} rejected: ${apv.title}`,
    ...
    return NextResponse.json({ id: apv.id, status: "rejected" });
  } catch (err) {
