...
const Icons = {
  ...
  layout: (
    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 3h7a2 2 0 012 2v14a2 2 0 01-2 2h-7m0-18H5a2 2 0 00-2 2v14a2 2 0 002 2h7m0-18v18"/></svg>
  ),
  building: (
...
const kpiData = [
  ...
  { label: "Open payments", value: "C$188K", meta: "2 awaiting collection", icon: "dollar", color: "green" },
  { label: "Compliance", value: "1", meta: "", trend: "1 expiring", trendType: "warn", icon: "shield", color: "amber" },
  { label: "RFIs overdue", value: "2", meta: "of 6 open", trend: "action needed", trendType: "warn", icon: "fileText", color: "red" },
];

const priorities = [
  { title: "Resolve CO-14 client approval block", desc: "Mechanical reroute waiting on owner signoff — now affecting procurement timing.", pill: "Urgent", pillColor: "red", time: "3d open", urgent: true },
  { title: "Review Northline electrical submittal", desc: "Revised package uploaded this morning, needs PM review before end of day.", pill: "Review", pillColor: "blue", time: "Today" },
  { title: "Release weekly owner summary", desc: "Progress photos and milestone notes ready — approval language still needs review.", pill: "Comms", pillColor: "amber", time: "Draft ready" },
  { title: "West End Medical — bid responses follow-up", desc: "Two subcontractor bids still outstanding past the 48-hour response window.", pill: "Follow up", pillColor: "gray", time: "Overdue" },
];

const projectHealth = [
  { name: "Riverside Tower Fit-Out", desc: "Client decision lag affecting two connected workstreams.", pill: "Watch", pillColor: "amber", phase: "Phase 3" },
  { name: "West End Medical", desc: "Stable progress, no major escalation in 48 hours.", pill: "Healthy", pillColor: "green", phase: "Phase 2" },
  { name: "Lot 7 Redevelopment", desc: "Early planning moving but bid responses slower than expected.", pill: "Monitor", pillColor: "amber", phase: "Precon" },
  { name: "Northline Office Park", desc: "On schedule — structural framing completing this week.", pill: "On track", pillColor: "green", phase: "Phase 1" },
];

const approvalsData = [
  { title: "CO-14 owner signoff", desc: "Mechanical reroute", pill: "Blocked", pillColor: "red" },
  { title: "Lighting package review", desc: "Client decision pending", pill: "Open", pillColor: "amber" },
];
...
export default function BuiltCRMShell() {
...
.b-srch-k{position:absolute;right:8px;top:50%;transform:translateY(-50%);font-family:var(--fm);font-size:10px;font-weight:500;color:var(--t3);background:var(--s2);padding:2px 5px;border-radius:4px;border:1px solid var(--s3)}
...
.b-top{height:var(--th);display:flex;align-items:center;justify-content:space-between;padding:0 var(--sp,24px);border-bottom:1px solid var(--s3);background:var(--tb-bg);backdrop-filter:blur(12px);position:sticky;top:0;z-index:50}
...
.b-kpi-l{font-family:var(--fd);font-size:11.5px;font-weight:700;color:var(--t2);text-transform:uppercase;letter-spacing:.04em}
...
.b-trend{font-weight:720}.b-trend.up{color:var(--ok-t)}.b-trend.warn{color:var(--wr-t)}
...
.b-ctt{font-family:var(--fd);font-size:15px;font-weight:740;color:var(--t1);letter-spacing:-.01em}
...
.b-row p{font-size:12.5px;font-weight:500;color:var(--t2);margin:2px 0 0;line-height:1.4}
.b-rm{display:flex;align-items:center;gap:8px;flex-shrink:0;padding-top:1px}

/* pills */
.b-pill{height:22px;padding:0 8px;border-radius:999px;font-size:11px;font-weight:720;display:inline-flex;align-items:center;white-space:nowrap;font-family:var(--fb)}
...
.b-met-v{font-family:var(--fd);font-size:18px;font-weight:800;color:var(--t1);margin-top:2px;letter-spacing:-.02em}
...
.b-cnt>*:nth-child(1){animation-delay:0ms}.b-cnt>*:nth-child(2){animation-delay:50ms}.b-cnt>*:nth-child(3){animation-delay:100ms}
...
@media(max-width:1100px){.b-kpis{grid-template-columns:repeat(3,1fr)}.b-dg{grid-template-columns:1fr}.b-drl{display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:16px}}
@media(max-width:768px){.b-app{grid-template-columns:1fr}.b-sb{position:fixed;top:0;left:0;bottom:0;width:var(--sw);transform:translateX(-100%);box-shadow:var(--shlg)}.b-sb.open{transform:translateX(0)}.b-ov{display:block}.b-menu{display:grid}.b-top{padding:0 16px}.b-cnt{padding:16px}.b-kpis{grid-template-columns:repeat(2,1fr)}.b-ph{flex-direction:column;gap:12px}.b-pa{width:100%}.b-pa .b-btn{flex:1;justify-content:center}.b-pt{font-size:22px}}
@media(max-width:480px){.b-kpis{grid-template-columns:1fr}.b-drl{grid-template-columns:1fr}}
      `}</style>
      ...
      <div className="b-app">
        ...
        <main className="b-main">
          <div className="b-top" style={{padding:"0 24px"}}>
          ...
          <div className="b-cnt">
            ...
            <section className="b-kpis">
              {kpiData.map((k)=>(
                <div key={k.label} className={`b-kpi ${k.label==="RFIs overdue"?"alert":""}`}>
            ...
            <section className="b-dg">
              <div className="b-dm">
                <div className="b-c">
                  ...
                  <div className="b-cb">{priorities.map((p,i)=>(<div key={i} className={`b-row ${p.urgent?"urg":""}`}><div><h5>{p.title}</h5><p>{p.desc}</p></div><div className="b-rm"><span className={`b-pill ${p.pillColor}`}>{p.pill}</span><span className="b-time">{p.time}</span></div></div>))}</div>
                </div>
                <div className="b-c">
                  <div className="b-ch"><div><div className="b-ctt">Project health</div><div className="b-cst">Status across your active projects</div></div></div>
                  <div className="b-cb">{projectHealth.map((p,i)=>(<div key={i} className="b-row"><div><h5>{p.name}</h5><p>{p.desc}</p></div><div className="b-rm"><span className={`b-pill ${p.pillColor}`}>{p.pill}</span><span className="b-time">{p.phase}</span></div></div>))}</div>
                </div>
              </div>
              <div className="b-drl">
                <div className="b-c alert">
                  <div className="b-ch"><div><div className="b-ctt">Approvals waiting</div><div className="b-cst">Holding release steps</div></div></div>
                  <div className="b-cb">{approvalsData.map((a,i)=>(<div key={i} className="b-row"><div><h5>{a.title}</h5><p>{a.desc}</p></div><div className="b-rm"><span className={`b-pill ${a.pillColor}`}>{a.pill}</span></div></div>))}</div>
                </div>
                <div className="b-c">
                  <div className="b-ch"><div><div className="b-ctt">Quick health</div></div></div>
                  <div className="b-cb"><div className="b-mets">{quickHealth.map((m,i)=>(<div key={i} className="b-met"><div className="b-met-l">{m.label}</div><div className="b-met-v">{m.value}</div><div className="b-met-h">{m.hint}</div></div>))}</div></div>
                </div>
                <div className="b-c">
                  ...
                  <div className="b-cb">{messagesData.map((m,i)=>(<div key={i} className="b-row"><div><h5>{m.from}</h5><p>{m.text}</p></div><div className="b-rm"><span className="b-time">{m.time}</span></div></div>))}</div>
                </div>
              </div>
            </section>
          </div>
