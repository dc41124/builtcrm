...
const F = { display:"'DM Sans',system-ui,sans-serif", body:"'Instrument Sans',system-ui,sans-serif" };
const CHK = <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M20 6 9 17l-5-5"/></svg>;
const ARR = <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14"/><path d="m12 5 7 7-7 7"/></svg>;

const FEATURES = [
  { icon:<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8Z"/><path d="M14 2v6h6"/><path d="M16 13H8"/><path d="M16 17H8"/><path d="M10 9H8"/></svg>, color:"#5b4fc7", title:"AIA-format billing", desc:"G702/G703 draw requests with schedule of values, retainage tracking, and automated lien waiver collection. Your accountant will thank you." },
  { icon:<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>, color:"#2a7f6f", title:"RFIs & issue tracking", desc:"Log field questions, assign to the right trade, track responses with full history. No more texts that disappear into the void." },
  { icon:<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>, color:"#3178b9", title:"Client portals", desc:"Commercial owners see approvals, draws, and documents. Homeowners see progress photos, selections, and budgets — each in a tone that fits." },
  { icon:<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 5H2v7l6.29 6.29a1 1 0 0 0 1.42 0l5.58-5.58a1 1 0 0 0 0-1.42Z"/><circle cx="6" cy="9" r="1"/></svg>, color:"#3d6b8e", title:"Selections management", desc:"Create allowance-based categories, curate options with photos and specs, and let clients choose — with automatic budget impact tracking." },
  { icon:<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>, color:"#c17a1a", title:"Compliance tracking", desc:"Insurance, licenses, certifications — track it all per sub with expiration alerts. Non-compliant subs see a restricted view until they're current." },
  { icon:<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>, color:"#2d8a5e", title:"Schedule & milestones", desc:"Phase-based timelines with milestone tracking, client-visible schedules, and subcontractor scope views. Everyone knows what's next." },
];

const SOLUTIONS = {
  gc: { color:"#5b4fc7", title:"Your command center for every active project", desc:"BuiltCRM gives GCs a single dashboard across all projects — with deep tools for billing, subs, and client communication built in.", features:[
  ...
  sub: { color:"#3d6b8e", title:"Show up, do the work, get paid — less admin", desc:"Subs get a clean Today Board that surfaces everything they need across all their active projects. No hunting through emails.", features:[
  ...
  commercial: { color:"#3178b9", title:"Real-time visibility into your investment", desc:"Commercial property owners and tenant reps get a professional portal for approvals, financial tracking, and project transparency.", features:[
  ...
  residential: { color:"#2a7f6f", title:"Your home build, demystified", desc:"Homeowners get a warm, clear portal to follow along with their build — from selections to photos to payments, in language that makes sense.", features:[
...
export default function MarketingWebsite() {
  ...
  const [audience, setAudience] = useState("gc");
  const [annual, setAnnual] = useState(true);
  const [resCat, setResCat] = useState("all");
  const [activeArticle, setActiveArticle] = useState(null);

  const nav = (p) => { setPage(p); setActiveArticle(null); window.scrollTo({ top:0, behavior:"smooth" }); };
      ...
      <nav style={{ position:"sticky", top:0, zIndex:100, background:"rgba(250,249,247,.85)", backdropFilter:"blur(16px)", borderBottom:"1px solid #eeece8", padding:"0 32px" }}>
        <div style={{ maxWidth:1200, margin:"0 auto", height:64, display:"flex", alignItems:"center", justifyContent:"space-between", gap:32 }}>
          <div onClick={()=>nav("home")} style={{ display:"flex", alignItems:"center", gap:10, cursor:"pointer" }}>
            <div style={{ width:34, height:34, borderRadius:10, background:"linear-gradient(135deg,#2c2541,#5b4fc7)", display:"grid", placeItems:"center" }}><svg viewBox="0 0 80 80" width="19" height="19"><rect x="14" y="14" width="26" height="26" rx="4" fill="none" stroke="white" strokeWidth="3.5" opacity=".5"/><rect x="26" y="26" width="26" height="26" rx="4" fill="none" stroke="white" strokeWidth="3.5" opacity=".75"/><rect x="32" y="32" width="26" height="26" rx="4" fill="white" opacity=".95"/></svg></div>
            <div style={{ fontFamily:F.display, fontSize:19, fontWeight:780, letterSpacing:"-.04em" }}>BuiltCRM</div>
          </div>
          <div style={{ display:"flex", alignItems:"center", gap:4 }}>
            {[["home","Product"],["solutions","Solutions"],["pricing","Pricing"],["resources","Resources"]].map(([k,l])=>(
          ...
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            <button style={{ height:38, padding:"0 16px", fontSize:13.5, fontWeight:620, color:"#5e5850", background:"transparent", border:"none", borderRadius:10, cursor:"pointer" }}>Log in</button>
            <button style={{ height:38, padding:"0 20px", fontSize:13.5, fontWeight:650, color:"white", background:"#5b4fc7", borderRadius:10, display:"flex", alignItems:"center", gap:6, border:"none", cursor:"pointer" }}>Get started free <span style={{ width:14, height:14, display:"block" }}>{ARR}</span></button>
          </div>
        </div>
      </nav>
        ...
        <div>
          ...
          <section style={{ padding:"100px 32px 80px", textAlign:"center", position:"relative", overflow:"hidden" }}>
            ...
            <div style={{ maxWidth:800, margin:"0 auto", position:"relative", zIndex:1 }}>
              ...
              <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:12, flexWrap:"wrap" }}>
                <Btn lg primary>Start free trial <span style={{ width:16, height:16, display:"inline-block" }}>{ARR}</span></Btn>
                <Btn lg secondary>Watch demo</Btn>
              </div>
            </div>
          </section>
          ...
          <Section dark>
            <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:24, textAlign:"center" }}>
              {[["3","Portal experiences"],["12+","Workflow modules"],["100%","AIA-compliant billing"],["0","Lost text messages"]].map(([n,l],i)=>(
                <div key={i}>
          ...
          <Section>
            <div style={{ textAlign:"center", maxWidth:640, margin:"0 auto" }}>
              ...
              <div style={{ display:"flex", gap:12, justifyContent:"center", flexWrap:"wrap" }}>
                <Btn lg primary>Get started free <span style={{ width:16, height:16, display:"inline-block" }}>{ARR}</span></Btn>
                <Btn lg secondary>Talk to sales</Btn>
              </div>
            </div>
          </Section>
        </div>
        ...
        <div>
          <Section style={{ paddingTop:56 }}>
            ...
            <div style={{ display:"flex", justifyContent:"center", gap:6, marginBottom:48, flexWrap:"wrap" }}>
              {[["gc","General Contractors","#5b4fc7"],["sub","Subcontractors","#3d6b8e"],["commercial","Commercial Owners","#3178b9"],["residential","Homeowners","#2a7f6f"]].map(([k,l,c])=>(
                <button key={k} onClick={()=>setAudience(k)} style={{ height:40, padding:"0 20px", borderRadius:999, fontSize:13.5, fontWeight:620, color:audience===k?"white":"#5e5850", background:audience===k?c:"transparent", border:audience===k?"none":"1px solid #e5e2dc", display:"flex", alignItems:"center", gap:8, cursor:"pointer", transition:"all 120ms" }}>
          ...
          <Section alt>
            <div style={{ textAlign:"center", maxWidth:640, margin:"0 auto" }}>
              ...
              <div style={{ display:"flex", gap:12, justifyContent:"center", flexWrap:"wrap" }}>
                <Btn lg primary>Get started free</Btn>
                <Btn lg secondary>See pricing</Btn>
              </div>
            </div>
          </Section>
        </div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:20, maxWidth:960, margin:"0 auto" }}>
              {PRICING.map((p,i)=>(
                <div key={i} style={{ background:"white", border:p.featured?"2px solid #5b4fc7":"1px solid #e5e2dc", borderRadius:20, padding:"32px 28px", position:"relative", boxShadow:p.featured?"0 0 0 1px #5b4fc7, 0 12px 40px rgba(26,23,20,.08)":"none" }}>
                  ...
                  <button style={{ width:"100%", height:42, borderRadius:10, fontSize:14, fontWeight:650, display:"flex", alignItems:"center", justifyContent:"center", gap:6, marginBottom:24, border:p.ctaPrimary?"none":"1px solid #e5e2dc", background:p.ctaPrimary?"#5b4fc7":"white", color:p.ctaPrimary?"white":"#1a1714", cursor:"pointer" }}>{p.cta}</button>
        ...
        <div>
          <Section style={{ paddingTop:56 }}>
            ...
            <div style={{ display:"flex", justifyContent:"center", gap:6, marginBottom:36, flexWrap:"wrap" }}>
              {[["all","All"],["blog","Blog"],["guide","Guides"],["case-study","Case studies"]].map(([k,l])=>(
                <button key={k} onClick={()=>setResCat(k)} style={{ height:34, padding:"0 14px", borderRadius:999, fontSize:13, fontWeight:620, color:resCat===k?"#faf9f7":"#5e5850", background:resCat===k?"#1a1714":"transparent", border:resCat===k?"none":"1px solid #e5e2dc", cursor:"pointer" }}>{l}</button>
              ))}
            </div>
            <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:24 }}>
              {RESOURCES.filter(r=>resCat==="all"||r.cat===resCat).map((r,i)=>{
                const catColor = { blog:"#5b4fc7", guide:"#2a7f6f", "case-study":"#3178b9" }[r.cat];
          ...
          <div style={{ position:"fixed", inset:0, zIndex:9999, background:"#faf9f7", overflowY:"auto" }}>
            ...
            <article style={{ maxWidth:720, margin:"0 auto", padding:"48px 24px 80px" }}>
              ...
              <div style={{ marginTop:56, padding:"40px 32px", background:"linear-gradient(135deg,#2c2541,#5b4fc7)", borderRadius:20, textAlign:"center" }}>
                ...
                <button onClick={()=>{setActiveArticle(null);nav("pricing");}} style={{ height:44, padding:"0 28px", fontSize:14, fontWeight:650, color:"#1a1714", background:"white", borderRadius:12, border:"none", cursor:"pointer", display:"inline-flex", alignItems:"center", gap:8 }}>See plans and pricing <span style={{ width:14, height:14, display:"block" }}>{ARR}</span></button>
              </div>
            </article>
          </div>
        );
      ...
      <footer style={{ background:"#2c2541", color:"rgba(250,249,247,.7)", padding:"72px 32px 36px" }}>
        <div style={{ maxWidth:1200, margin:"0 auto" }}>
          <div style={{ display:"grid", gridTemplateColumns:"2fr 1fr 1fr 1fr", gap:64, marginBottom:56 }}>
            <div>
              <div style={{ width:34, height:34, borderRadius:10, background:"linear-gradient(135deg,#5b4fc7,#7c6fe0)", display:"grid", placeItems:"center", marginBottom:14 }}><svg viewBox="0 0 80 80" width="19" height="19"><rect x="14" y="14" width="26" height="26" rx="4" fill="none" stroke="white" strokeWidth="3.5" opacity=".5"/><rect x="26" y="26" width="26" height="26" rx="4" fill="none" stroke="white" strokeWidth="3.5" opacity=".75"/><rect x="32" y="32" width="26" height="26" rx="4" fill="white" opacity=".95"/></svg></div>
...
/* ── Helpers ── */
function Section({ children, alt, dark, style={} }) {
  return (
...
function Btn({ children, lg, primary, secondary }) {
  return (
    <button style={{
