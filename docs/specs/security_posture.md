...
| `dsarRequests` | Anonymous-write surface | Quebec Law 25 §32 mandates that *anyone* can submit a DSAR — including former users with deleted accounts and authorized representatives who were never users. The public POST at `/privacy/dsar` is unauthenticated, sets no GUC, and would be denied by an `organization_id = current_org_id()` RLS policy. The trade considered: (a) RLS with a `current_org_id IS NULL` carve-out for anonymous inserts (fragile — hides the policy seam), (b) a separate "intake bucket" table with a triage step that rewrites into a real RLS'd table (more migration weight + dual-write risk during triage), or (c) skip RLS and lean on app-layer scoping. Picked (c): every read site filters by `organization_id` explicitly via Drizzle (`where(eq(dsarRequests.organizationId, ctx.org))`), the public POST goes through `dbAdmin` to signal cross-cutting intent, and RLS audit's allowlist tracks `dsarRequests` so a future `db.*` call site without an explicit org filter trips CI. The same RLS exemption does NOT extend to `consent_records` or `breach_register` (Session C) — those are admin-only and will be RLS'd normally. |

**Why this matters:** A future contributor reviewing the audit script (`scripts/_audit-rls-coverage.mjs`) will see 9 + 2 + 1 = 12 tables un-RLS'd and may try to "fix" them. This subsection is the canonical answer to "why isn't `auditEvents` RLS'd" — don't add it, the system writers depend on cross-org INSERT.
